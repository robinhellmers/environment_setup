...
modify_path()
{
    local position="$1"  # "append" or "prepend"
    local new_path="$2"

    case "$position" in
        'append'|'prepend') ;;
        *)
            echo "ERROR: modify_path - Invalid position: '$position'"
            return 1
            ;;
    esac
    ...
    if [[ ! -d "$new_path" ]]
    then
        echo "ERROR: $position - Path is not a directory: '$new_path'"
        return 1
    fi
